...
QRawObjectList RawReader::read_weapons(QFileInfo path, int start) {
    QRawObjectList result;
    ...
    if(file.open(QIODevice::ReadOnly)) {
        ...
        int N = start;
        while(!lines.isEmpty()) {            
            RawObjectPtr obj = read_weapon(lines, N);
...
RawObjectPtr RawReader::read_weapon(QStringList & lines, int N) {
    RawObjectPtr result;
    ...
    QString key = "[ITEM_WEAPON:";
    if(line.trimmed().startsWith(key)) { //[ITEM_WEAPON:ITEM_WEAPON_NAME]
        result = RawObjectPtr(new RawObject);
        populate_weapon_node_values(result, line, QString("%1").arg(N));
        populate_weapon_sub_nodes(result, lines);
        TRACE << "Read object [" << result->get_name() << ":" << result->get_id() << "]";
    }
    return result;
}

void RawReader::populate_weapon_node_values(const RawNodePtr & node, QString & line, QString id) {
    QString str = line.trimmed();
    str = str.mid(1, str.indexOf("]")-1); //ITEM_WEAPON:ITEM_WEAPON_NAME
    QStringList strings = str.split(":", QString::SkipEmptyParts); //ITEM_WEAPON ITEM_WEAPON_NAME
    node->name = strings[0]; //ITEM_WEAPON
    strings.insert(1, id);
    for(int i = 1; i < strings.size(); ++i) {
        node->values.append(strings[i]); //ITEM_WEAPON_NAME
    }
}

void RawReader::populate_weapon_sub_nodes(const RawNodePtr & node,  QStringList & lines) {
    bool done = false;
    while(!done && !lines.isEmpty()) {
        QString line = lines.front();
        lines.removeFirst();

        if(line.startsWith("[ITEM_WEAPON:")) {
            done = true;
            lines.push_front(line);
        } else {
            if(line.trimmed().startsWith('[')) {
